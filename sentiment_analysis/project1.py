...
def classify(feature_matrix, theta, theta_0):
    ...
    # since Python lists and loops are slow.
    #
    # return np.array([classify_vector(v, theta, theta_0) for v in feature_matrix])
    raise NotImplementedError
...
def bag_of_words(texts, remove_stopword=False):
    ...
    """
    # Your code here
     # Load stopwords if `remove_stopword` is True
    stopwords = set()
    if remove_stopword:
...
def extract_bow_feature_vectors(reviews, indices_by_word, binarize=True):
    ...
    Args:
        ...
        binarize: If True, uses binary features; if False, uses word counts.
    Returns:
        A matrix representing each review via bag-of-words features. 
        The matrix has shape (n, m), where n is the number of reviews and m is the number of unique words.
    """
    feature_matrix = np.zeros([len(reviews), len(indices_by_word)], dtype=np.float64)
    for i, text in enumerate(reviews):
        word_list = extract_words(text)  # Tokenize the review into words
        for word in word_list:
            if word not in indices_by_word:
                continue

            if binarize:
                # Binary feature: 1 if the word appears in the review
                feature_matrix[i, indices_by_word[word]] = 1
            else:
                # Count feature: Increment by the number of occurrences of the word
                feature_matrix[i, indices_by_word[word]] += 1
